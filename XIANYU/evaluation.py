...
def eval(detection, ground_truth, img_root, show=True):
    ...
    for i, image_id in enumerate(detection):
        ...
        if i % 200 == 0:
            print('[%d/%d] TP:%d, FP:%d, FN:%d, Precesion:%.3f, Recall:%.3f, F1:%.3f' % (i, amount, TP, FP, FN, precesion, recall, f1))
...
no_text = True
only_text = False
# det_path = 'E:\\Mulong\\Result\\rico\\rico_xianyu\\rico_xianyu_bg_ocr'
det_path = 'E:\\Temp\\fse\\xianyu\\rico_xianyu_bg_ocr'
# det_path = 'E:\\Temp\\fse\\xianyu\\xianyu-old'
# det_path = 'E:\\Temp\\fse\\xianyu\\xianyu-h1000'
detect = load_detect_result_json(det_path, no_text=no_text, only_text=only_text)
gt = load_ground_truth_json('E:\\Mulong\\Datasets\\gui\\rico\\instances_test.json', no_text=no_text, only_text=only_text)
eval(detect, gt, 'E:\\Mulong\\Datasets\\gui\\rico\\combined\\all', show=False)
