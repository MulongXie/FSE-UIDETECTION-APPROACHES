...
def compo_merge(corner_a, corner_b):
...
def compo_relation(compo_a, compo_b):
    ...
    """
    col_min_a, row_min_a, col_max_a, row_max_a = compo_a
    col_min_b, row_min_b, col_max_b, row_max_b = compo_b
...
def merge_intersected_compo(compos):
    changed = False
    new_compos = []
    for i in range(len(compos)):
        merged = False
        for j in range(len(new_compos)):
            relation = compo_relation(compos[i], compos[j])
            if relation == 2:
                new_compos[j] = compo_merge(new_compos[j], compos[i])
                merged = True
                changed = True
                break
        if not merged:
    ...
    if not changed:
        ...
        return merge_intersected_compo(new_compos)
