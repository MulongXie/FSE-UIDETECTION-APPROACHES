...
def generateProject(imageLocation, output_root, show=False):
    ...
    for i, key in enumerate(compos):
        ...
        compos_json['compos'].append({
            'id': i, 'class': class_map[compo.mType],
        ...
        cv2.rectangle(img_org, (compo.x, compo.y), (compo.x + compo.width, compo.y + compo.height), color=(0,255,0), thickness=2)
    # print(annotation)
    json.dump(compos_json, open(pjoin(output_root, mProjectName + '.json'), 'w'))
...
def remaui(input_path_img, output_root):
    ...
    try:
        generateProject(input_path_img, output_root, show=True)
